...
Please read \citew[Chapter~7]{MuellerGermanic}.
...
\item Such non-nominative-subjects are also called \alert{quirky subjects} oder \alert{oblique subjects}.
\end{itemize}
...
\begin{itemize}
\item Verbs agree with the nominative element. \\
			If there is none, the verb is third person singular (neuter).

\pause

\item no agreement in (\mex{1}a):
...
\frame{
\frametitle{Quirky subjects in the passive: position}
...
\frame{
\frametitle{Quircky subjects in the passive: Control}
...
only need one lexical entry for the verb  \emph{lesen} `to read' in (\mex{1}):

\eal
\ex 
\gll \blau{Er} möchte das Buch lesen.\\
     he.\NOM{} wants the.\ACC{} book read \\\german
\glt `He wants to read the book.'
					
\ex 
\gll Ich      sah \blau{ihn} das Buch lesen. \\
     I.\NOM{} saw him.\ACC{} the book read\\
...
\eal
\ex[]{
\gll Wir gedenken \blau{der} \blau{Opfer}. \\
     we.\NOM{} remember the victims.\GEN{}\\
\glt `We remember the victims.'
}
		
\ex[]{
\gll \blau{Der} \blau{Opfer} wird gedacht. \\
...
\frame{
\frametitle{Hypothese 2: The dative passiv with bivalent verbs -- I}
...
\frame{
\frametitle{Hypothese 2: The dative passiv with bivalent verbs -- II}
...
So: Dative passive has to be done differently somehow if dative is lexical with two-place verbs.

You may assume hypothesis 1 right away: All datives are lexical.
...
\frame{
\frametitle{The Case Principle (I)}
...
a. & \emph{schläft} `sleeps':     & \argst \sliste{ NP[\type{str}]$_i$ }\\
b. & \emph{unterstützt} `supports': & \argst \sliste{ NP[\type{str}]$_i$, NP[\type{str}]$_j$ }\\
c. & \emph{hilft} `helps':       & \argst \sliste{ NP[\type{str}]$_i$, NP[\type{ldat}]$_j$ }\\
d. & \emph{schenkt} `gives as present':     & \argst \sliste{ NP[\type{str}]$_i$, NP[\type{ldat}]$_j$, NP[\type{str}]$_k$ }\\
\end{tabular}
\z
\pause
...
Passivizing the verbs results in the following \argst-lists for \emph{wird}:

\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
a. & \emph{geschlafen wird} `slept was':  & \argst \sliste{ V }\\
b. & \emph{unterstützt wird} `supported was': & \argst \sliste{ NP[\type{str}]$_j$, V }\\
c. & \emph{geholfen wird} `helped was':    & \argst \sliste{ NP[\type{ldat}]$_j$, V }\\
d. & \emph{geschenkt wird} `given was':   & \argst \sliste{ NP[\type{ldat}]$_j$, NP[\type{str}]$_k$, V }\\
\end{tabular}
\z
...
\frame{
\frametitle{Impersonal passives in Danish: Expletives}
...
\eal
	...
	\gll fordi manden giver \rot{drengen} \gruen{bolden}\\ 
	     because man.{\sc def} gives boy.{\sc def} ball.{\sc def}\\
...
\ex\label{ex-boy-was-given-ball-danish}
	\gll fordi \rot{drengen} bliver givet \gruen{bolden}\\ 
	     because boy.{\sc def} \AUX{} given ball.{\sc def}\\
...
\ex\label{ex-ball-was-given-boy-danish}
	\gll fordi \gruen{bolden} bliver givet \rot{drengen}\\ 
	     because ball.{\sc def} \AUX{} given boy.{\sc def}\\
	\glt `because the ball was given to the boy'
...
\pause
\item Side note: the verb always agrees with nominative.
...
c.&schenken (give as p.): & \sliste{\ibox{1}NP[\type{str}], NP[\type{ldat}], NP[\type{str}]}& \sliste{\ibox{1}}\\[2mm]
...
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{}}
  &                     & {\sc arg-st}\\[2mm]
a.&danse   (dance, unerg):     & \liste{NP[\type{str}]}\\[2mm]
b.&læse      (read, trans):   & \liste{NP[\type{str}], NP[\type{str}]}\\[2mm]
c.&give      (give, ditrans): & \liste{NP[\type{str}], NP[\type{str}], \blaubf{NP[\type{str}]}}\\[2mm]
d.&hjælpe    (help, trans):   & \liste{NP[\type{str}], \blaubf{NP[\type{str}]}}\\
\end{tabular}
%}
\z
...
\frame[shrink=15]{
\frametitle{Generalized lexical rule}
...
\frame{
\frametitle{Result of the lexical rule application for Danish}
...
\frame{
\frametitlefit{Complements of non-finite verbs can be fronted with the verb}

%\savespace
%\smallexamples

Complements can be fronted together with the verb:
...
\frame{
\frametitlefit{Subjects of non-finite verbs cannot be fronted with the verb}



However, the subject normally can not:
...
\frame{
\frametitle{Participles can be fronted with all arguments}


Participles allow for the fronting of both objects of the active verb:
\eal
	\ex 
...
\begin{itemize}
\item Lexical rule also works for the morphological passive. A \suffix{s} is simply added.
\end{itemize}
...
\pause
\item The designated argument is blocked, but is contained in the lexical item.
\pause
...
\frame{
\frametitle{Remote passive and verbal complex formation (I)}
...
\frame{
	\frametitle{Remote passive and verbal complex formation (II)}
	
\begin{itemize}
\item Remote passive only possible with verbal complexes:
\eal
	\ex[]{
...
\item Explanation: Remote passive = passivization of the predicate complex
...
\frame{
\frametitle{Remote passive with object control verbs}

\smallframe 
\smallexamples

\begin{itemize}
\item Remote passive also possible with object control verbs:
...
\pause
\item SVO languages require different lexical items for perfect/passive participles, but for
